...
პროექტის ვერიფიკაციისა და კოდის მდგრადობის (Robustness) შესამოწმებლად, რეპოზიტორიის ძირეულ საქაღალდეში არსებულ `test_data/` ფოლდერში განთავსებულია 5 სხვადასხვა კატეგორიის სატესტო Excel ფაილები. თითოეული კატეგორია ამოწმებს სისტემის კონკრეტულ ბიზნეს-ლოგიკასა და ვალიდაციის მექანიზმებს:

| კატეგორია (საქაღალდე) | მიზანი და აღწერა | მოსალოდნელი შედეგი ინტერფეისში |
| :--- | :--- | :--- |
| **`1. standard`** | იდეალური და სუფთა სატესტო მონაცემები ყოველგვარი ხარვეზის გარეშე. | სისტემა წარმატებით აგენერირებს ოპტიმიზირებულ შეკვეთას და ინახავს მონაცემებს ბაზაში. |
| **`2. incorrect_file_name`** | ფაილები, რომელთა სახელებიც არ შეესაბამება სისტემის სტანდარტს (მაგ. `sales_corrupted.xlsx`). | FastAPI აჩერებს პროცესს და ფრონტენდზე გამოაქვს ერორი: *"ფაილის სახელი არასწორია"*. |
| **`3. null_value`** | ფაილები, სადაც გარკვეულ უჯრედებში მონაცემები აკლია (ცარიელი ველები / `Null Value`). | სისტემის Data Cleaning მექანიზმი ავტომატურად ასუფთავებს ან ავსებს გამოტოვებულ ადგილებს ბიზნეს წესების მიხედვით. |
| **`4. spaces`** | ფაილები, სადაც ტექსტურ ველებში (მაგ. მაღაზიის ან მომწოდებლის სახელებში) ჩარჩენილია ზედმეტი გამოტოვებები (Spaces). | კოდში არსებული `.str.strip()` ავტომატურად ასუფთავებს "სფეისებს" და მონაცემები ბაზაში სუფთად იწერება. |
| **`5. big_data`** | დიდი მოცულობის რეალური საოპერაციო მონაცემები (სტრიქონების დიდი რაოდენობა). | იტესტება Pandas ძრავის სისწრაფე და სისტემის წარმადობა (Performance) დიდი მოცულობის მონაცემებზე. |
...
3. ატვირთეთ არჩეული ფოლდერიდან ხუთივე ფაილი შესაბამის ველებში (Current Stock, Sales, OnWay Stock, MinQ, Calendar).
